...
- The framework library and selection algorithms live in `assets/framework-pattern-library.md`.
...
> **Operational copy:** `visual-mode.md` §5 is the operational EVOKE copy with grounding pre-check, avoid-list, and full dimension detail. `design_generation_patterns.md` is the upstream canon for anti-default rules. This section mirrors that structure so the two copies cannot drift.
