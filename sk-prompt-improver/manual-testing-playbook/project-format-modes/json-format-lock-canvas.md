...
Step 1 fixes the packaging under test. Step 2 binds Improve and locks JSON, delivering or asking once. Step 3 completes delivery when Turn 1 asked. Step 4 proves the header and attestation lines are present and the payload between them parses as valid JSON. A Turn 2 reply that follows a Turn 1 delivery is checked only for the blocking defects the root playbook lists.
