...
> **Not a direction chooser.** Each entry below names the vocabulary a recognizable category default speaks, so use it to RECOGNIZE the expected cliche, then articulate deliberate deviations. Reusing any direction unchanged across briefs is a preset and must not ship.
